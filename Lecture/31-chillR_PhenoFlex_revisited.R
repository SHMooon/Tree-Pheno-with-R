...
# make a constant model with function with different parameter. which is chill response
# 
apply_const_temp <-
  function(temp, A0, A1, E0, E1, Tf, slope, portions = 1200, deg_celsius = TRUE) {
    temp_vector <- rep(temp,
                       times = portions) # repeat many times: in here 1200 times
    res <- chillR::DynModel_driver(temp = temp_vector, #temps are with kalvin: physically better. 
                                              #without minus degree (vor Celsius there is minus dgree pretty early)
                                   A0 = A0, #form here they are parameters. 
                                   A1 = A1,
                                   E0 = E0,
                                   E1 = E1,
                                   Tf = Tf,
...
gen_bell <- function(par,
  ...
  y <- c()
  for(i in seq_along(temp_values)) { #sequance all the numbers 
  ...
  return(invisible(y)) #y is invisible now. 
}
...
GDH_response <- function(T, par)  {
  Tb <- par[11] #parameters: Tb, Tu, Tc_ [position]. 
   Tu <- par[4]
   Tc <- par[10]
...
ggplot(pivoted_response,
  ...
  geom_line(linewidth = 2,
            aes(col = name)) + # until here the chill and heat plots are on the same graph and hard to recognize 
  ...
  scale_color_manual(values = c("Chill_response" = "blue", #chilling to blue. 
                                "Heat_response" = "red")) + #heat to red 
  theme_bw(base_size = 15) +
  theme(legend.position = "none") #in this case we don't need a regend 

#the plots from the lecture: after 30 degree there is no data. thats why it cut. 
#chill respense start to end is about 7 degrees difference. 
...
month_range <- c(10, 11, 12, 1, 2, 3) #dormancy season.
...
for(mon in month_range) #work through all of the month
    {days_month <- as.numeric(difftime(ISOdate(2002, mon+1, 1), #days of month
                                       ISOdate(2002, mon, 1)))
     if(mon == 12) days_month <- 31 #December 31 days
     weather <-
       make_all_day_table(data.frame(Year = c(2002, 2002),
                                     Month = c(mon, mon),
                                     Day = c(1, days_month), #in this code contain only first and last day
                                     Tmin = c(0, 0),
                                     Tmax = c(0, 0)))

     for(tmin in Tmins)
      for(tmax in Tmaxs)
        if(tmax >= tmin) #max temp never be lower than min...
          {
           hourtemps <- weather %>%
             mutate(Tmin = tmin,
                    Tmax = tmax) %>%
             stack_hourly_temps(latitude = latitude) %>%
             pluck("hourtemps", "Temp") #pluck is from the tydiverse, we only want hourtemp from Temp part
             ...
             c(chill_eff,
               PhenoFlex(temp = hourtemps,
                         ...
                         basic_output = FALSE)$y %>% 
                          tail (1)/days) #divide total number of out dataset. 
...
#Model_sensitivities_PhenoFlex: this have no obs... so check it!! 
...
#the stripe from the plot red & yellow. maybe a phenomenon that happens in KA
#never be happen the chilling lower that 0 degree according to this parameter set. 

...
function (climate_scenario_list = chills_GWANGJU, metric = "Chill_Portions", metric_label = "Chill (Chill Portions)", 
          year_name = "End_year", 
          label_sides = "both", ylim = c(0, NA), reference_line = NULL, 
          col_rect = NA, col_line = NA, hist_col = NA, texcex = 2, 
          caption_above = FALSE, family = "serif", no_scenario_numbers = FALSE) 
{
      ...
      if (length(plotlabels) == length(plotdata)) {
        ...
        if (no_scenario_numbers) 
          axis(1, labels = FALSE, at = labelpos[which(labelpos > 
               ...
               tck = -0.05)
        else axis(1, at = labelpos[which(labelpos > 0)], 
